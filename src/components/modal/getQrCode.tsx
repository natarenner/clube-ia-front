...
export const GetQrCode: React.FC = () => {
  ...
  const fetchQrCode = async () => {
    setSubmitting(true);
    try {
      const url = `${process.env.NEXT_PUBLIC_STARKS_EVO_BASE_URL!}/refresh-code/instance?instanceName=${instanceName}`;
  ...
  const verifyConnection = async () => {
    setVerifying(true);
    try {
      const url = `${process.env.NEXT_PUBLIC_STARKS_EVO_BASE_URL!}/get-connection/instance?instanceName=${instanceName}`;
      ...
      if (status === 'closed') {
        ...
        toast({
          ...
          description: `Por favor escaneie o QR Code ou gere um novamente.`
        });
      } else {
        // toast({
        //     variant: 'default',
    ...
    <>
      {qrCodeBase64 && (
        <div className="p-8 mb-8 space-y-4 flex flex-col">
          ...
          <Image src={qrCodeBase64} alt="QR Code" width={320} height={320} className="mx-auto" />
          <Button
            onClick={fetchQrCode}
